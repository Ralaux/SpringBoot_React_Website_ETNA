...
public class ControllerTests {
    ...
    public void testUser() throws Exception {
    	...
    	.perform(
    			...
    			.content("{\"username\":\"TestUser3\"}"))
    	.andDo(print())
    	.andExpect(MockMvcResultMatchers.jsonPath("$.username").value("TestUser3"))
    	.andExpect(status().isOk());
    	...
    	this.mockMvc
    	.perform(
    			MockMvcRequestBuilders.delete("/user/" + user2.getId()).header("Authorization", "Bearer " + token2))
    	.andDo(print())
    	.andExpect(status().isForbidden());
